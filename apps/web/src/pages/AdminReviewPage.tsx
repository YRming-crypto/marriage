...
export function AdminReviewPage() {
  ...
  const [contentForm, setContentForm] = useState({ type: "article" as "article" | "event", title: "", summary: "", body: "", tags: "", startsAt: "", endsAt: "", location: "", capacity: "" });
  ...
  function createContentInput(): CreateAdminContentInput | null {
    ...
    if (base.type === "article") return base;
    ...
    return { ...base, event: { startsAt, endsAt, location: contentForm.location.trim(), capacity } };
  }
  ...
  async function saveContentDraft() {
    ...
    try {
      ...
      setContentForm({ type: "article", title: "", summary: "", body: "", tags: "", startsAt: "", endsAt: "", location: "", capacity: "" });
  ...
  return <div className="page-shell shell admin-review">
    ...
    {tab === "content" && !tabLoading && !tabError && contents ? <div className="admin-content-layout"><section className="admin-panel"><div className="admin-panel__heading"><div><small>编辑区</small><h2>新建内容</h2></div></div><form className="admin-content-form" onSubmit={(event) => { event.preventDefault(); void saveContentDraft(); }}><label><span>内容类型</span><select aria-label="内容类型" value={contentForm.type} onChange={(event) => setContentForm((current) => ({ ...current, type: event.target.value as "article" | "event" }))}><option value="article">文章</option><option value="event">线下活动</option></select></label><label><span>内容标题</span><input aria-label="内容标题" maxLength={100} value={contentForm.title} onChange={(event) => setContentForm((current) => ({ ...current, title: event.target.value }))} /></label><label><span>内容摘要</span><textarea aria-label="内容摘要" rows={3} maxLength={300} value={contentForm.summary} onChange={(event) => setContentForm((current) => ({ ...current, summary: event.target.value }))} /></label><label><span>内容正文</span><textarea aria-label="内容正文" rows={8} value={contentForm.body} onChange={(event) => setContentForm((current) => ({ ...current, body: event.target.value }))} /></label><label><span>内容标签</span><input aria-label="内容标签" value={contentForm.tags} onChange={(event) => setContentForm((current) => ({ ...current, tags: event.target.value }))} placeholder="多个标签用逗号分开" /></label>{contentForm.type === "event" ? <div className="admin-event-fields"><label><span>开始时间</span><input aria-label="活动开始时间" type="datetime-local" value={contentForm.startsAt} onChange={(event) => setContentForm((current) => ({ ...current, startsAt: event.target.value }))} /></label><label><span>结束时间</span><input aria-label="活动结束时间" type="datetime-local" value={contentForm.endsAt} onChange={(event) => setContentForm((current) => ({ ...current, endsAt: event.target.value }))} /></label><label><span>活动地点</span><input aria-label="活动地点" value={contentForm.location} onChange={(event) => setContentForm((current) => ({ ...current, location: event.target.value }))} /></label><label><span>人数上限</span><input aria-label="活动人数上限" type="number" min="1" value={contentForm.capacity} onChange={(event) => setContentForm((current) => ({ ...current, capacity: event.target.value }))} /></label></div> : null}<button className="button button--primary" type="submit" disabled={busyAction === "create-content"}><FileClock aria-hidden="true" />保存草稿</button></form></section><section className="admin-panel"><div className="admin-panel__heading"><div><small>内容库</small><h2>内容管理</h2><p>草稿、已发布和已下线内容都会保留在这里。</p></div><button className="button button--soft" type="button" onClick={() => void loadTab("content", true)}><RefreshCw aria-hidden="true" />刷新</button></div><div className="admin-content-list">{contents.map((content) => <article key={content.id}><div><div className="admin-item-list__meta"><span className={`admin-status admin-status--${content.status}`}>{contentStatusText[content.status]}</span><span>{content.type === "event" ? "活动" : "文章"}</span></div><h3>{content.title}</h3><p>{content.summary}</p>{content.tags.length ? <small>{content.tags.join(" · ")}</small> : null}</div><div className="admin-actions">{content.status !== "published" ? <button className="button button--primary" type="button" disabled={busyAction === `content-${content.id}`} onClick={() => void changeContentStatus(content, "publish")}><Check aria-hidden="true" />发布</button> : <button className="button button--soft" type="button" disabled={busyAction === `content-${content.id}`} onClick={() => void changeContentStatus(content, "offline")}><Power aria-hidden="true" />下线</button>}</div></article>)}</div>{!contents.length ? <ServiceState icon={BookOpenText} title="还没有内容" detail="可在左侧创建第一篇文章或活动草稿。" /> : null}</section></div> : null}
